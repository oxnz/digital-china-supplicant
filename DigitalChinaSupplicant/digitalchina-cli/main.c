...
#define LOCKFILE "/var/run/dcclient.pid"        /* 锁文件 */
...
daemon_init(void)
{
    ...
    int     fd0;
        ...
        fprintf(stdout, ">>Info: DCClient Forked background with PID: [%d]\n", pid);
                exit(0);
    }
...
program_running_check()
{
    ...
    if (exit_flag) {
        if (fl.l_type != F_UNLCK) {
            ...
            fprintf (stderr, ">>Info: NO DCClient Running.\n");
        exit (EXIT_FAILURE);
    }
...
int main(int argc, char **argv)
{
    ...
    if ( (ins_pid = program_running_check ()) ) {
        fprintf(stderr,"@@ERROR: DCClient Already "
                            "Running with PID %d\n", ins_pid);
        exit(EXIT_FAILURE);
    }
